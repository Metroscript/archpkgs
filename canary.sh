...
sudo swapon -a
free -m
sudo sed -i 's/#resume=/resume=/' /efi/loader/entries/arch.conf
...
sudo sed -i 's/splash/splash vt.global_cursor_default=0' /efi/loader/entries/arch.conf
...
sudo cp /efi/loader/entries/arch.conf /efi/loader/entries/lts.conf
sudo sed -i -e 's/vmlinuz-linux/vmlinuz-linux-lts' -i -e 's/initramfs-linux/initramfs-linux-lts' /efi/loader/entries/lts.conf
