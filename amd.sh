...
sudo pacman -Syyu --needed wayland mesa pipewire-{pulse,jack,media-session} plasma-{wayland-session,nm,desktop,pa,firewall,disks,thunderbolt,vault,systemmonitor} kde{plasma-addons,-gtk-config} xfce4-terminal k{gamma5,infocenter,screen,screenlocker,menuedit,wayland-integration,eepassxc} powerdevil gimp discover sddm{,-kcm} nemo{,-fileroller} gparted spectacle qbittorrent rhythmbox lollypop x{ed,reader} libreoffice-still gnome-calculator mpv print-manager simple-scan oxygen vulkan-{radeon,icd-loader} wine gnutls lib{pulse,32-{gnutls,libpulse,pipewire{,-jack,-v4l2},mesa,vulkan-{radeon,icd-loader}}} steam flatpak virtualbox{,-host-modules-arch} firewalld neofetch
